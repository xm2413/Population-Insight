...
- 全球人口统计：按国家和洲维度统计国家统计局与世界银行公开人口数据
- 地区对比：多地区同年份横向对比
...
首次启动会自动创建 `data/population_trend.db`，并写入北京、上海、广东、四川、河南等地区公开统计资料整理数据，以及中国、印度、美国、印度尼西亚、巴西、尼日利亚等国家维度的公开人口数据。中国数据采用国家统计局年度统计公报口径，其他国家采用世界银行公开数据。后续启动会自动同步内置公开数据，避免旧版本数据残留。
...
其中人口单位为万人；出生率、死亡率、自然增长率按千分率口径填写；老龄化率、城镇化率按百分比口径填写。
...
- `database.py`：数据库连接、建表、初始化公开统计数据
